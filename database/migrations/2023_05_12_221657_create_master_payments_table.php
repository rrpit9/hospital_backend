...
class CreateMasterPaymentsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('master_payments', function (Blueprint $table) {
            ...
            $table->string('gateway',30)->comment('The payment gateway name');
            $table->string('gateway_status', 30)->nullable()->comment('The payment status returned by the gateway');
            $table->string('gateway_order_id', 70)->nullable()->comment('The payment gateway/merchant order id');
            $table->string('gateway_payment_id', 70)->nullable()->comment('The payment gateway payment id');
            ...
            $table->decimal('amount', 10,2)->comment('The final payment amount charged');
            $table->decimal('gateway_fee', 10,2)->nullable()->comment('Gateway fee including all the taxes and additional charges collected by payment gateway.');
            $table->decimal('gateway_gst_rate', 6,2)->nullable()->comment('GST rate charged by the payment gateway');
            $table->decimal('gateway_gst_amount', 10,2)->nullable()->comment('GST amount charged by the payment gateway');
